...
function PillSpec() {
  ...
  const { pillCode } = useParams(); // useParams 훅을 사용하여 URL에서 pillCode를 가져옵니다.

  useEffect(() => {
    // 서버에서 데이터를 가져오는 요청
    axios.get(`http://110.12.181.206:8081/pillSpec/?pillCode=${pillCode}`)
      .then(response => {
  ...
  }, [pillCode]); // pillCode가 변경될 때마다 useEffect가 다시 실행됩니다.
